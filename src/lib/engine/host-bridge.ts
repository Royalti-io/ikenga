...
export function createShellAcpHost(): AcpHost {
	return {
		initialize: (req) => chatInitialize(req),
		newSession: (req) => chatNewSession(req),
		prompt: (req) => chatPrompt(req),
		cancel: (sessionId) => chatCancel(sessionId),
		setMode: (sessionId, modeId) => chatSetMode(sessionId, modeId),
		loadSession: (sessionId) => chatLoadSession(sessionId),
		forkSession: (sourceSessionId, opts) => chatForkSession(sourceSessionId, opts),
		listenSession: (sessionId, onUpdate) => chatListen(sessionId, onUpdate),
		listenPermissionRequests: (sessionId, onRequest) => chatListenRequests(sessionId, onRequest),
		respondPermission: (requestId, response) => chatRespondPermission(requestId, response),
		listenNotify: (callback) => chatListenNotify(callback),
	};
}
