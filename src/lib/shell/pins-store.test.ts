  ...
  return {
    ...
    activityPinsAdd: vi.fn(async (args: Record<string, unknown>) => {
      ...
      const sortOrder = state.pins.filter((p) =>
        (args.sectionId ?? null) === null
          ? p.sectionId === null
          : p.sectionId === args.sectionId,
      ).length;
      const pin = {
        id: `pin-${state.nextId++}`,
        kind: args.kind,
...
const resetMockState = (cmd as unknown as { __resetMockState: () => void })
  .__resetMockState;
...
describe('pins store — section creation flow', () => {
  ...
  it('rejects creating a section with a reserved id', async () => {
    ...
    await store.hydrate();
    await expect(
      store.createSection({ id: 'system', label: 'System' }),
    ).rejects.toThrow();
    await expect(
      store.createSection({ id: 'settings', label: 'Settings' }),
    ).rejects.toThrow();
  });
});
...
describe('pins store — reorder', () => {
  ...
  it('reverts state on Rust error', async () => {
    ...
    await expect(
      usePinsStore.getState().reorderPins([a.id], 'finance'),
