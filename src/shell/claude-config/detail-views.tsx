...
function DetailHead({
	...
	return (
		<div className="ccfg-detail-head">
			...
			<h2>
				{title}
				<span className={cn('ccfg-scope-pill', scope === 'project' ? 'is-project' : 'is-personal')}>
					{scope}
				</span>
			</h2>
...
export function AgentDetailView({ agent, onEdit, onNewSession }: AgentDetailProps) {
	...
	const fmRows = fmEntries(agent.frontmatter, ['name', 'model', 'maxTurns']);
	fmRows.push(
		[
			'allowed-tools',
	...
	return (
		<>
			...
			<div className="ccfg-detail-body">
				...
				{skillsUsed.length > 0 && (
					<Section label="Skills used" count={`${skillsUsed.length} of ${skillsUsed.length}`}>
						<Chips values={skillsUsed} variant="skill" />
					</Section>
				)}
				{tools.length > 0 && (
					<Section label="Allowed tools" count={`${Math.min(tools.length, 8)} of ${tools.length}`}>
	...
	const fmRows: Array<[string, ReactNode]> = [
		...
		[
			...
			allowed.length ? (
				...
				<span style={{ color: 'var(--fg-faint)' }}>— (inherits caller)</span>
			),
		],
	...
	return (
		<>
			<DetailHead
				...
				actions={
					<IconButton onClick={() => onEdit(skill.path)}>
						<Edit3 size={11} /> Edit
					</IconButton>
				}
			/>
...
export function CommandDetailView({ cmd, onEdit, onRun }: CommandDetailProps) {
	...
	const fmRows: Array<[string, ReactNode]> = [
		...
		[
			'allowed-tools',
			allowed.length ? (
				`${tools.length} tools · ${mcp.length} MCP`
			) : (
				<span style={{ color: 'var(--fg-faint)' }}>—</span>
			),
		],
	...
	return (
		<>
			<DetailHead
				...
				actions={
					<>
						<IconButton onClick={() => onEdit(cmd.path)}>
							<Edit3 size={11} /> Edit
						</IconButton>
			...
			<div className="ccfg-detail-body">
				...
				<Section label="Body · the prompt that runs" count={`${cmd.body.split('\n').length} lines`}>
		...
		<>
			...
			<div className="ccfg-detail-body">
				...
				<Section label="settings path">
					<FilePathLink path={hook.settingsPath} onClick={onEdit} className="ccfg-filepath" />
				</Section>
				{hook.commandPath && (
					<Section
						label="Script body"
					...
					>
						{scriptBody ? (
							...
							<pre className="ccfg-script" style={{ color: 'var(--danger)' }}>
								{scriptError}
							</pre>
						) : loadAttempted ? (
							...
							<div style={{ color: 'var(--fg-faint)', fontSize: 12 }}>
								...
								<FilePathLink path={hook.commandPath} onClick={onEdit} className="ccfg-filepath" />.
							</div>
						)}
					</Section>
				)}
...
export function McpDetailView({ mcp, onEdit }: McpDetailProps) {
	...
	if (isStdio) {
		...
		fmRows.push(['url', mcp.url ?? <span style={{ color: 'var(--fg-faint)' }}>—</span>]);
	}
	...
	return (
		<>
			<DetailHead
				...
				actions={
					<IconButton onClick={() => onEdit(mcp.path)}>
						<Edit3 size={11} /> Edit
					</IconButton>
				}
			/>
...
function TransportBadge({ transport }: { transport: string }) {
	const variant =
		transport === 'http' || transport === 'sse' ? 'is-mcp' : transport === 'stdio' ? 'is-tool' : '';
