 ...
 * during dogfooding.
 */
export function EventList({ events, className }: { events: ChatEvent[]; className?: string }) {
		...
		case 'thinking':
			return (
				<Row icon={Brain} tone="muted" label="thinking">
					<p className="whitespace-pre-wrap text-xs italic text-muted-foreground">{event.delta}</p>
				</Row>
			);
		case 'tool_use':
			return (
...
function coalesceText(events: ChatEvent[]): ChatEvent[] {
	...
	for (const e of events) {
		...
		if (last && last.kind === 'text' && e.kind === 'text') {
			out[out.length - 1] = { kind: 'text', delta: last.delta + e.delta };
			continue;
		}
		if (last && last.kind === 'thinking' && e.kind === 'thinking') {
