...
const PATH_RE = /^[~/]?[\w.@][\w./@_-]*\.[A-Za-z][A-Za-z0-9]{0,7}$/;

// Restrict to known dev/doc/asset extensions so things like `e.g.` or
// `Mr.A` don't get mistaken for files. Single-segment (no slash) paths
// must match this set; multi-segment paths fall through the regex with
// the standard checks. Lowercase for case-insensitive comparison.
...
async function resolvePathWithFallback(
  p: string,
  cwd: string | undefined,
): Promise<string> {
