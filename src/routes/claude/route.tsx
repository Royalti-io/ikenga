    ...
    <div className="flex h-full flex-col p-3">
      <div className="ccfg flex-1 min-h-0">
        <Header
          counts={counts}
          onReload={() =>
            router.invalidate({ filter: (m) => m.routeId.startsWith('/claude') })
          }
...
function Header({
  ...
  counts:
    | { agents: number; skills: number; commands: number; hooks: number; mcps: number }
    | null;
  onReload: () => void;
  onOpenDir: () => void;
}) {
  return (
    <div
      className="flex items-center justify-between border-b border-[var(--border-soft)] bg-[var(--bg-surface)] px-4 py-3"
    >
      <div>
        <div
