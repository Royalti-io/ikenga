    ...
    const waitForInitialize = (timeoutMs: number) =>
      new Promise<boolean>((resolve) => {
        ...
        const tick = () => {
          ...
          try {
            const w = f?.contentWindow as unknown as { __iframeapp_state?: { connected: boolean } } | null;
    ...
    (async () => {
      ...
      if (phase === 'longlived') {
        ...
        try {
          ...
          log('LL_SEQUENTIAL', seqOk ? `OK 1→${av}, 2→${bv}` : `FAIL ${a.error ?? ''} ${b.error ?? ''} av=${av} bv=${bv}`);
        } catch (e) {
        ...
        try {
          ...
          log('LL_UNINSTALL', teardownOk ? 'OK (supervisor entry cleared)' : `FAIL still present: ${JSON.stringify(after)}`);
        } catch (e) {
      ...
      if (phase === 'hyperframes') {
        ...
        try {
          ...
          if (r.ok && Array.isArray(projects) && projects.length > 0 && typeof projects[0] === 'string') {
        ...
        try {
          ...
          contentUrlOk =
            handle.url.startsWith('http://127.0.0.1:') && handle.token.length === 64;
          ...
          try {
            ...
            const state = await waitFor(
              () => {
                ...
                return s && s.currentSlug === firstSlug && typeof s.port === 'number'
                  ? s
                  : null;
              },
              OPEN_TIMEOUT_MS,
        ...
        try {
          ...
          const after = await findSupEntry();
          teardownOk = after === null;
          log('HF_UNINSTALL', teardownOk ? 'OK (supervisor entry cleared)' : `FAIL still present: ${JSON.stringify(after)}`);
        } catch (e) {
      ...
      if (phase === 'video-studio') {
        ...
        try {
          ...
          log(
            'VS_START',
            startOk ? `OK port=${port}` : `FAIL ${r.error ?? `port=${port}`}`,
          );
        } catch (e) {
        ...
        try {
          ...
          const after = await findSupEntry();
          teardownOk = after === null;
          log('VS_UNINSTALL', teardownOk ? 'OK (supervisor entry cleared)' : `FAIL still present: ${JSON.stringify(after)}`);
        } catch (e) {
      ...
      if (phase === 'storyboard') {
        ...
        try {
          ...
          const after = await findSupEntry();
          teardownOk = after === null;
          log('SB_UNINSTALL', teardownOk ? 'OK (supervisor entry cleared)' : `FAIL still present: ${JSON.stringify(after)}`);
        } catch (e) {
      ...
      if (phase === 'storyboard-recovery') {
        ...
        type SupEntry = { pkg_id: string; state: string; pid?: number | null; last_err?: string | null };
        type SupSnap = { entries?: SupEntry[] };
        const findSupEntry = async (): Promise<SupEntry | null> => {
          const status = await pkgKernelStatus();
          const sup = (status.registries.sidecar_supervisor ?? {}) as SupSnap;
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'monospace', fontSize: 13 }}>
      ...
      <div style={{ marginBottom: 8, fontSize: 12, opacity: 0.7 }}>
        phase=<b>{phase}</b> — try ?phase=cleanup | install | verify | roundtrip | longlived | hyperframes | video-studio | storyboard
      </div>
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} style={{ borderTop: '1px solid #333' }}>
              <td style={{ padding: '4px 12px 4px 0', opacity: 0.7, verticalAlign: 'top' }}>{r.label}</td>
      ...
      {mountSrc && (
        ...
        >
          <PkgIframeHost
            ...
            onInitialized={() => { initializedRef.current = true; }}
          />
        </div>
      )}
    </div>
