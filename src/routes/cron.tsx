// /cron — discoverability deep-link into com.ikenga.agent-ops "Schedule" view.
//
// The agent-ops pkg is a top-level activity-bar app whose primary home is
// /pkg/com.ikenga.agent-ops/. The Schedule view lives under that route tree.
...
// sub-path (e.g. /pkg/com.ikenga.agent-ops/schedule), upgrade this to a
// direct redirect so /cron → the live Schedule view with no intermediate UI.
...
// The pkg root path; WP-08 will replace this with a /schedule sub-path.
		...
		(async () => {
			try {
				...
				if (!cancelled && installed) {
					...
					// switching to the Schedule view. WP-08 will wire a direct
					// sub-path once the pkg declares it.
