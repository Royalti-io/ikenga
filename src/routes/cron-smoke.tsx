    ...
    (async () => {
      ...
      try {
        ...
        const reg = s.registries.cron as
          | { entries: Array<{ pkg_id: string; cron_id: string; expr: string; handler: string }>; count: number }
          | undefined;
        const mine = (reg?.entries ?? []).filter((e) => e.pkg_id === PKG_ID);
        log('REGISTRY', `count=${reg?.count ?? 'n/a'} pkg_jobs=${mine.length} ${JSON.stringify(mine.map((m) => `${m.cron_id} ${m.expr}→${m.handler}`))}`);
      } catch (e) {
      ...
      try {
        ...
        log('POST-UNINSTALL', `before=${before} after=${after} delta=${after - before} stopped=${stoppedOk}`);
      } catch (e) {
