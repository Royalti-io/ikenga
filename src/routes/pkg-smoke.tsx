...
function PkgSmoke() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      ...
      const sidecars = s.registries.sidecars as { entries: Array<{ pkg_id: string; name: string }>; count: number };
    ...
    (async () => {
      ...
      try {
        ...
        log('STATUS', `installed=${r.installedRow} sidecar_registered=${r.sidecarRegistered} api_version=${r.apiVersion}`);
      } catch (e) {
        log('STATUS', `FAIL ${(e as Error).message ?? String(e)}`);
      }
      ...
      if (phase === 'install') {
        ...
        if (!cancelled) setVerdict(ok ? 'VERDICT PASS (left installed for boot replay)' : 'VERDICT FAIL');
