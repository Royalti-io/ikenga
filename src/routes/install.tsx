...
function InstallPanel() {
	...
	useEffect(() => {
		...
		iykeEndpoint()
			.then(setIyke)
			.catch((e) => console.warn('[install] iyke endpoint', e));
	}, []);
			...
			<div className="inst-page-body">
				...
				<div className="inst-row">
					...
					<div className="inst-source-pane">
						{sourceKind === 'catalog' && (
							<CatalogGrid catalog={catalog} value={catalogChoice} onChange={setCatalogChoice} />
						)}
						{sourceKind === 'local' && (
							...
							<>
								...
								<div>
									<div className="inst-blocks-label">
										Declared blocks
										<span className="sub">· {declaredBlocks.length} of 8 registries touched</span>
									</div>
									{declaredBlocks.length ? (
										<div className="inst-blocks-stack">
											{declaredBlocks.map((b) => (
												<div key={b.kind} className={cls('inst-block-row', `is-${b.kind}`)}>
					...
					<div className="inst-inspect-right">
						{!manifest ? (
							...
							<div className="inst-perm-empty">
								...
								<p>
									Skill-only package — no fs/net/shell access. The kernel still records the install
									with an empty <code>permissions</code> block. <em>Intentional · not a bug.</em>
								</p>
							</div>
						) : (
							<>
				...
				<div className="inst-row inst-act-grid">
						...
						{settingsSchema.length === 0 ? (
							...
							settingsSchema.map((f) => (
								<SettingsFieldRow
									...
									onChange={(v) => setSettingsOverrides((p) => ({ ...p, [f.key]: v }))}
									onToggleSecret={() =>
					...
					<div className="inst-act-right">
						<div className="inst-handoff">
							...
							<p>
								The kernel registers; Claude does the acquisition. Same prompt is used whether you
								open it in-app or paste it into a terminal.
							</p>
...
function RowRule(props: { num: number; label: string }) {
	return (
		<div className="inst-row-rule">
			...
			{props.label}
			<svg className="carve" width="48" height="6" viewBox="0 0 48 6" aria-hidden="true">
