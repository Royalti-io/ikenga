	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				setState(
					entry.kind === 'iframe' ? { kind: 'iframe', entry } : { kind: 'unmountable', entry }
				);
			} catch (e) {
		...
		return (
			<div className="p-6 text-sm">
				...
				<div className="opacity-70">
					<code>pkg://{pkgId}</code> is not installed, or declares no UI routes. Open the package
					manager to install or inspect.
				</div>
			</div>
		);
	}
