...
pub async fn get_browser_list(
    ...
    let entries: Vec<ListEntry> = panes
        .statuses()
        .into_iter()
...
pub async fn post_browser_screenshot() -> Result<Json<Value>, (StatusCode, String)> {
    Err((
        StatusCode::NOT_IMPLEMENTED,
        "browser screenshot is Phase 4+ — the existing screenshot command targets the main \
         window/shell pane, not pkg-owned child webviews. Track in STATUS.md."
            .into(),
    ))
}
// Suppress unused-warning for the timeout constant until screenshot lands.
#[allow(dead_code)]
...
pub async fn post_browser_focus(
    JsonBody(_body): JsonBody<PaneKey>,
) -> Result<Json<OkResponse>, (StatusCode, String)> {
    // Kernel doesn't expose focus today (macOS/Windows in-window children
    // and Linux borderless top-levels both need different OS-level affordances).
    // Returning ok preserves the tool surface; document as a no-op for v1.
    Ok(ok())
}
